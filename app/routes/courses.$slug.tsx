...
import { Link, useSearchParams, Form, redirect } from "react-router";
...
export function meta({ data: loaderData }: Route.MetaArgs) {
  ...
  return [
    { title: `${title} — TLabs Learning Hub` },
...
export async function action({ params, request }: Route.ActionArgs) {
  ...
  const currentUserId = await getCurrentUserId(request);
  if (!currentUserId) {
    throw redirect(`/signup?redirectTo=${encodeURIComponent(`/courses/${slug}`)}`);
  }
...
export default function CourseDetail({ loaderData }: Route.ComponentProps) {
  ...
  const enrollButton = currentUserId ? (
    ...
    <Link to={`/signup?redirectTo=${encodeURIComponent(`/courses/${course.slug}`)}`}>
      <Button size="lg" className="w-full">
        Enroll Now
      </Button>
    </Link>
