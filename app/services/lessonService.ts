import { eq, and, sql, gt, lt, gte, lte, inArray } from "drizzle-orm";
import { db } from "~/db";
import {
  lessons,
  videoWatchEvents,
...
export function deleteLesson(id: number) {
  // 1. Delete direct lesson dependencies
  db.delete(videoWatchEvents).where(eq(videoWatchEvents.lessonId, id)).run();
  db.delete(lessonProgress).where(eq(lessonProgress.lessonId, id)).run();

  // 2. Handle Quizzes (which have their own nested dependencies)
  const quizIds = db
  ...
  if (quizIds.length > 0) {
    ...
    db.delete(quizzes).where(eq(quizzes.lessonId, id)).run();
  }
